	...
	public DataSource getCurrentDataSource() {
		if (dataSource == null) {
			synchronized (BasicDAO.class) {
				if (dataSource == null) {
					if (dsName == null) {
						setDataSource((DataSource)TableFieldXmlsParser.getBean(StmtParams.getInstance().getDataSource()));
					} else {
						setDataSource((DataSource)TableFieldXmlsParser.getBean(dsName));
					}
				}
			}
		}
